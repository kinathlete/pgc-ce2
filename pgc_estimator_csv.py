...
sim_replaced_dc = st.sidebar.selectbox(label="Data Center to be replaced",options=filtered_em_pd['region'].unique(),\
    help="Select a data center that you would like to swap out with another data center",)
sim_replacing_dc = st.sidebar.selectbox(label="Replacing data center",options=raw_reg_pd["region"],\
...
with st.container():
    ...
    with col11:
        st.markdown(":point_right: Services Used: **{}**".format(filtered_em_pd['rate_id'].count()))
    with col12:
        st.markdown(":point_right: Products Serviced: **{}**".format(filtered_em_pd['service_id'].nunique()))
    with col13:
        st.markdown(":point_right: Data Centers: **{}**".format(filtered_em_pd['data_center'].nunique()))
    with col14:
        st.markdown(":point_right: Duration (d): **{}**".format(filtered_em_pd['day'].nunique()))
...
with st.container():
    st.subheader("CO2 Emissions Over Time (t)")
    # st.dataframe(build_agg_e_by_dim(full_em_pd, 'day', 'sum'))
    st.line_chart(data=build_agg_e_by_dim(filtered_em_pd, 'day', 'sum'), x='day', y='co2_e', use_container_width=True)
